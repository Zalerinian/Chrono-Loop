        ...
        private void saveLevel(string file) {
            ...
            using (XmlWriter writer = XmlWriter.Create(file, settings)) {
                ...
                foreach (ToolObject tObj in higharchy) {
                    ...
                    if(tObj.mComponentList.Count > 0)
                    {
                        writer.WriteStartElement("ComponentModel");
                        for(int i = 0; i < tObj.mComponentList.Count; ++i)
                        {
                            tObj.mComponentList[i].WriteComponentData(writer);
                        }
                        writer.WriteEndElement();
                    }
                                ...
                                case XmlNodeType.Element:
                                    switch (reader.Name) {
                                        ...
                                        case "Collider":
                                            ...
                                            break;
                                        case "ComponentModel":
                                            // Read in the Component Model stuff while it lasts.
                                            while(reader.Read())
                                            {
                                                ...
                                                } else if(reader.NodeType == XmlNodeType.Element)
                                                {
                                                    switch(reader.Name)
                                                    {
                                                        case "ParticleEmitter":
                                                            ParticleEmitterComponent p = new ParticleEmitterComponent(addition.mComponentList);
                                                            p.ReadComponentData(reader);
                                                            break;
                                                        default:
                                                            break;
                                                    }
