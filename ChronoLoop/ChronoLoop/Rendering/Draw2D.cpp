...
void Draw::InitializeDirect2D()
{
	...
	RenderEngine::Renderer::Instance()->ThrowIfFailed(D2D1CreateFactory(D2D1_FACTORY_TYPE_SINGLE_THREADED, &factory2));
	sInstance->mTextFactory = std::make_shared<ID2D1Factory1*>(factory2);

	//createDxgiDevice
	IDXGIDevice* DxgiDevice;
	RenderEngine::Renderer::Instance()->ThrowIfFailed((*RenderEngine::Renderer::Instance()->iGetDevice())->QueryInterface(__uuidof(IDXGIDevice), (void **)&DxgiDevice));
	sInstance->mGIDevice = std::make_shared<IDXGIDevice*>(DxgiDevice);

	//create device2d 
	ID2D1Device* Device2d;
	RenderEngine::Renderer::Instance()->ThrowIfFailed(factory2->CreateDevice(*sInstance->mGIDevice, &Device2d));
	sInstance->mDevice2D = std::make_shared<ID2D1Device*>(Device2d);

	//create context
	ID2D1DeviceContext* context2d;
	RenderEngine::Renderer::Instance()->ThrowIfFailed((*sInstance->mDevice2D)->CreateDeviceContext(D2D1_DEVICE_CONTEXT_OPTIONS_NONE, &context2d));
	sInstance->mContext2D = std::make_shared<ID2D1DeviceContext*>(context2d);

	IDWriteFactory* WriteFactory;
	RenderEngine::Renderer::Instance()->ThrowIfFailed(DWriteCreateFactory(DWRITE_FACTORY_TYPE_SHARED, __uuidof(WriteFactory), reinterpret_cast<IUnknown**>(&WriteFactory)));
...
void Draw::InitializeScreenBitmap()
{
	...
	sInstance->mScreenBitmap = std::make_shared<ID2D1Bitmap1*>(CreateBitmapForTexture(*RenderEngine::Renderer::Instance()->iGetRTViewTexture()));
}
//////
...
ID2D1SolidColorBrush * Draw::CreateNewBrush(D2D1::ColorF _color)
{
	...
	RenderEngine::Renderer::Instance()->ThrowIfFailed((*mContext2D)->CreateSolidColorBrush(_color, &brush));
...
IDWriteTextFormat * Draw::CreateNewTextFormat(Font _font)
{
	...
	RenderEngine::Renderer::Instance()->ThrowIfFailed((*mDWrite)->CreateTextFormat(
		_font.mFontName,
		NULL,
	...
	Font _font, std::wstring _text, ID2D1Bitmap* _bitmap)
{
	...
	RenderEngine::Renderer::Instance()->ThrowIfFailed((*mContext2D)->EndDraw());
...
void Draw::DrawRectangleToBitmap(float _left, float _top, float _right, float _bottom, D2D1::ColorF _color, ID2D1Bitmap * _bitmap)
{
	...
	ID2D1SolidColorBrush* brush;
	RenderEngine::Renderer::Instance()->ThrowIfFailed((*mContext2D)->CreateSolidColorBrush(_color, &brush));//D2D1::ColorF(D2D1::ColorF::LightGray,0.7f)
...
ID2D1Bitmap1 * Draw::CreateBitmapForTexture(ID3D11Texture2D * _texture) {
	...
	RenderEngine::Renderer::Instance()->ThrowIfFailed(_texture->QueryInterface(IID_IDXGISurface, (void**)&surface));

	ID2D1Bitmap1* bitmap;
	RenderEngine::Renderer::Instance()->ThrowIfFailed((*mContext2D)->CreateBitmapFromDxgiSurface(surface, &bitmapProperties, &bitmap));
	return bitmap;
}
