...
struct CCElasticAABBtoAABB : public CodeComponent
{
	...
	virtual void OnCollision(Collider& _col, Collider& _other, float _time)
	{
		if (!colliding && _other.mColliderType != Collider::eCOLLIDER_Plane)
		{
			...
			vec4f norm;
			if ((center.x <= max.x && center.x >= min.x) && V.z > 0)
				norm = { 0,0,1,0 };
			else if ((center.y <= max.y && center.y >= min.y) && V.y > 0 && V.y > max.y)
				norm = { 0,1,0,0 };
			else if ((center.z <= max.z && center.z >= min.z) && V.x > 0)
				norm = { 1,0,0,0 };
			else if ((center.x <= max.x && center.x >= min.x) && V.z < 0)
				norm = { 0,0,-1,0 };
			else if ((center.y <= max.y && center.y >= min.y) && V.y < 0 && V.y < min.y)
				norm = { 0,-1,0,0 };
			else if ((center.z <= max.z && center.z >= min.z) && V.x < 0)
				norm = { -1,0,0,0 };
			////////////////////////////////////////////////////Add corner cases
			...
			//vec4f collisionNormal = (center - point);
